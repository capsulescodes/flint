...
{
    INIT_CWD="$TEST" output=$( sh "$path/dist/init.sh" 2>&1 ) > /dev/null
    [ "$?" -eq 0 ]
...
{
    INIT_CWD="$TEST" PWD="$path" sh "$path/dist/init.sh" > /dev/null
    [ -d "$TEST/.flint" ]
...
{
    INIT_CWD="$TEST" PWD="$path" sh "$path/dist/init.sh" --with-hooks > /dev/null
    [ -d "$TEST/.flint/hooks" ]
...
it_handles_missing_source_files()
{
    ...
    output=$( INIT_CWD="$TEST" PWD="$path" sh "$TEST/dist/init.sh" 2>&1 ) > /dev/null
...
it_adds_git_wrapper_to_shell_config()
{
    ...
    INIT_CWD="$TEST" PWD="$path" SHELL="/bin/bash" HOME="$TEST/home" sh "$path/dist/init.sh" > /dev/null
...
it_handles_unknown_shell()
{
    ...
    INIT_CWD="$TEST" PWD="$path" SHELL="/bin/unknown" HOME="$TEST/home" sh "$path/dist/init.sh" > /dev/null
